...
func _update_transform(aabb=null):
	...
	if v.major == 3 and v.minor <= 1:
		...
		trans = Transform3D(Basis(), 0.5 * Vector3(width, height, depth) + Vector3(0, aabb.position.y, 0))
	else:
		# In 3.2, vertical centering changed.
		# https://github.com/godotengine/godot/pull/28326
		trans = Transform3D(Basis(), 0.5 * Vector3(width - 1, 0, depth - 1))
	...
	#PhysicsServer3D.body_set_shape_transform(_body_rid, 0, trans)
