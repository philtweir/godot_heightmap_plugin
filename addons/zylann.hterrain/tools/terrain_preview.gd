...
@onready var _mesh_instance = $SubViewport/MeshInstance3D
@onready var _camera = $SubViewport/Camera3D
@onready var _light = $SubViewport/DirectionalLight3D

# Use the simplest shader
var _shader = load("res://addons/zylann.hterrain/shaders/simple4_lite.gdshader")
var _yaw := 0.0
var _pitch := -PI / 6.0
var _distance := 0.0
...
func setup(heights_texture: Texture, normals_texture: Texture):
	...
	mat.set_shader_param("u_terrain_inverse_transform", Transform3D())
...
func cleanup():
	if _mesh_instance != null:
		var mat = _mesh_instance.mesh.surface_get_material(0)
		if mat != null: # RMV (otherwise error on startup)
